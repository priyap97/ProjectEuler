...
for i in range(20):
    for j in range(17):
        ...
        if x>numsAcross: numsAcross=x

for i in range(17):
    for j in range(20):
        x=grid[i][j]*grid[i+1][j]*grid[i+2][j]*grid[i+3][j]
        if x>numsDown: numsDown=x

for i in range(20):
    for j in range(20):
        if j-3<0 or i+3>=20: continue
        else:
            x=grid[i][j]*grid[i+1][j-1]*grid[i+2][j-2]*grid[i+3][j-3]
            if x>numsDiagLeft: numsDiagLeft=x

for i in range(20):
    for j in range(20):
        if j+3>=20 or i+3>=20: continue
        else:
            x=grid[i][j]*grid[i+1][j+1]*grid[i+2][j+2]*grid[i+3][j+3]
            if x>numsDiagLeft: numsDiagLeft=x

print(max([numsAcross,numsDown,numsDiagLeft,numsDiagRight]))
